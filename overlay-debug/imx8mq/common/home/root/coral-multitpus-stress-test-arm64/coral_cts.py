...
class TestSuite():
    ...
    def getTemp(self, TPU=2):
        while True:
            self._run_linux_system_command("TPU: apex_0 - Temp", ["cat", "/sys/class/apex/apex_0/temp"])
            if TPU == 2:
                self._run_linux_system_command("TPU: apex_1 - Temp", ["cat", "/sys/class/apex/apex_1/temp"])
            #self._read_temperatures("Multiple TPUs")
            time.sleep(1)

def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--output', default="cts.txt")
    parser.add_argument('--tpu', default=2)
    parser.add_argument('--inf', default="2147483647")
    parser.add_argument('--iteration', default="999999999")
    ...
    with TestSuite(output_file) as cts:
        ...
        t = threading.Thread(target=cts.getTemp, args=(args.tpu,))
        t.start()
        #if tpus > 1:
        cts.run_test(
                test=["multiple_tpus_inference_stress_test", "--num_inferences="+args.inf, "--gtest_filter=MultipleTpusStressTest.EfficientNetS", "--gtest_repeat="+args.iteration])
