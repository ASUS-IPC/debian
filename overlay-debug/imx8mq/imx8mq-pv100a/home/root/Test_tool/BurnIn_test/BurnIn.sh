...
select_test_item()
{
	...
	echo "9. COM1/COM2 RS232 stress test"
	echo "10.CAN bus stress test"
	echo "11.Audio stress test"
	echo "12.MCU DIO stress test"
	echo "13.COM3/COM4 RS232 stress test"
	echo "14.TPU stress test"
...
select_gps_module()
{
	...
	echo "2. Locosys GPS"
	echo
	read -p "Select GPS module: " gps_module
	gps_module=$[ $gps_module -1 ]
	echo gps_module=$gps_module
}
...
uart_1to2_stress_test()
{
	logfile=$LOG_PATH/uart.txt
	killall serial-test_loop.sh > /dev/null 2>&1
	killall serial-test > /dev/null 2>&1
	ex_gpio -c 1 232 > /dev/null 2>&1
	ex_gpio -c 2 232 > /dev/null 2>&1
	$SCRIPTPATH/test/serial-test_loop.sh 232 $SCRIPTPATH $logfile
}
...
audio_stress_test()
{
	...
	while true; do
		$SCRIPTPATH/../Audio_test/AudioTest.sh 2 1 10 $1
	done
}
...
mcu_dio_stress_test()
{
	...
	$SCRIPTPATH/test/mcu_dio_stress_test.sh $SCRIPTPATH $logfile
}
...
mcu_uart_stress_test()
{
	...
	ex_gpio -c 3 232 > /dev/null 2>&1
	ex_gpio -c 4 232 > /dev/null 2>&1
	$SCRIPTPATH/test/mcu_uart_stress_test.sh $SCRIPTPATH $logfile
}

mcu_dio_uart_stress_test()
{
	logfile=$LOG_PATH/mcu_dio_uart.txt
	killall mcu_dio_uart_stress_test.sh > /dev/null 2>&1
	ex_gpio -c 3 232 > /dev/null 2>&1
	ex_gpio -c 4 232 > /dev/null 2>&1
	$SCRIPTPATH/test/mcu_dio_uart_stress_test.sh $SCRIPTPATH $logfile
}
...
MCU_DIO_UART="mcu_dio_uart_stress_test.sh"
...
check_lte()
{
	...
	if [ "$Flag" == ""  ]
	then
		log "LTE stress test : ${COLOR_RED}stop${COLOR_REST} "
	else
		log "LTE stress test : ${COLOR_GREEN}running${COLOR_REST} "
	fi
}
...
check_gps()
{
	...
	if [ "$Flag" == "FAIL"  ]
	then
		log "GPS stress test : ${COLOR_RED}stop${COLOR_REST} "
	else
		log "GPS stress test : ${COLOR_GREEN}running${COLOR_REST} "
	fi
}
...
check_all_status()
{
	...
	check_wifi
	check_status UART $UART_1TO2
	check_status CAN $CAN
	check_status AUDIO $AUDIO
	check_status MCU_DIO_UART $MCU_DIO_UART
	check_lte
	check_gps
...
#		uart_stress_test ui
		;;
	10)	info_view CAN bus loopback
		can_stress_test
		;;
	11)	info_view Audio recored/playback
		audio_stress_test -a
		;;
	12)	info_view MCU DIO
		mcu_dio_stress_test
		;;
	13)	info_view MCU UART loopback
		mcu_uart_stress_test
		;;
	14)	info_view TPU
		tpu_stress_test
		;;
	15)	info_view MCU DIO UART
		mcu_dio_uart_stress_test
		;;
	*)
		check_system_status=true
		...
		uart_1to2_stress_test > /dev/null 2>&1 &
		can_stress_test > /dev/null 2>&1 &
		audio_stress_test -a  > /dev/null 2>&1 &
		mcu_dio_uart_stress_test  > /dev/null 2>&1 &
